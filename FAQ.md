...
* Android：[https://github.com/smart-test-ti/MagnaX/blob/master/DocForAndroid.md]()
...
* Android：使用的是adb连接，magnax自带有adb环境，但不一定适配每一台电脑；可以在自己的终端敲打adb devices测试是否连接上设备。
* iOS：使用 pymobiledevice3 进行设备连接和控制，如果连接失败可以参考官方文档（[https://github.com/doronz88/pymobiledevice3](https://github.com/doronz88/pymobiledevice3)）
...
## 6️⃣ 为什么"python -m magnax"会运行失败 ？

* magnax 2.5.4及以上版本只支持python 3.10 +的版本（因为使用了python新的特性，3.10以下版本不支持），2.5.3及以下版本支持python3.0 ~3.9。
* 如果显示的是50003端口被占用，可以用自定义的方式启动  `python -m magnax --host={ip} --port={port}` 。
* socket.gaierror: [Errno 8] nodename nor servname provided, or not known ：https://github.com/smart-test-ti/MagnaX/issues/198
...
* Android：pc和移动设备需要在同一个网络，然后通过adb connect的方式设置成功后，无须usb连接在magnax界面点击connect就会看到设备；具体方式可以网上查，很多教程。
* iOS：不支持。

## 9️⃣ 如何部署MagnaX ？

核心就是让“python -m magnax”这条命令在后台执行就可以。有很多种方式nohup、docker、Gunicorn等（可以网上查）

最简单的就是“nohup python -m magnax &”。

## 1️⃣0️⃣ MagnaX部署在云机器，远程访问本地的移动设备 ？

需要在连接移动设备的PC机器起magnax的服务，然后将host配置在MagnaX设置页的Anget中（右上角红点的设置按钮可查看）；

可以不用在同一个局域网，但是要保证本地的网络防火墙是放开的，可以让云机器访问。

## 1️⃣1️⃣ MagnaX如何获取微信小程序的性能 ？
...
* iOS：事先要装好模块“pip install py-ios-device”，然后点击Start（由于windows安装会报错，解决繁琐，magnax没有自带该模块）。
...
* Android：基本上新版本的系统已经不能通过adb的方式拿到能耗数据了，目前提供电量和温度两个指标其实已经足够；电量收集时magnax会在执行前断开充电，执行结束才恢复充电。
* iOS：选择能耗的方式来评估性能，这里可能会觉得充电会影响，但是我们测试是要和竞品对比，在相同条件下对比能耗即可，无需关注充电的影响。

## 1️⃣6️⃣ 如果iOS收集不到数据或报错，怎么解决 ？

* 插拔usb重新连接，多次尝试。
* 确保已正确安装 pymobiledevice3 和 py-ios-device 依赖。
* 确保设备已信任该电脑并启用开发者模式。
* iOS 17+ 设备需要先启动 tunnel 服务：`sudo python3 -m pymobiledevice3 remote start-tunnel`
* 如果仍有问题，可以参考官方文档进行排查：
...
- Android: 6.0+
- iOS: 基于pymobiledevice3实现，支持iOS 7.0+，具体兼容性请参考官方文档（https://github.com/doronz88/pymobiledevice3）
